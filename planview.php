...
$plan_view = NULL; // Initialize page object first

class cplan_view extends cplan {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["id_plan"] <> "") {
			$this->RecKey["id_plan"] = $_GET["id_plan"];
			$KeyUrl .= "&amp;id_plan=" . urlencode($this->RecKey["id_plan"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_plan"] <> "") {
				...
				$this->RecKey["id_plan"] = $this->id_plan->QueryStringValue;
			} else {
				$sReturnUrl = "planlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "planlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "planlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($plan_view)) $plan_view = new cplan_view();
...
$plan_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var plan_view = new ew_Page("plan_view");
plan_view.PageID = "view"; // Page ID
var EW_PAGE_ID = plan_view.PageID; // For backward compatibility

// Form object
var fplanview = new ew_Form("fplanview");

// Form_CustomValidate event
fplanview.Form_CustomValidate = 
...
fplanview.ValidateRequired = true;
<?php } else { ?>
fplanview.ValidateRequired = false; 
<?php } ?>
...
<?php $plan_view->ExportOptions->Render("body") ?>
<?php if (!$plan_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($plan_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $plan_view->ShowPageHeader(); ?>
<?php
$plan_view->ShowMessage();
?>
<form name="fplanview" id="fplanview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="plan">
<table class="ewGrid"><tr><td>
<table id="tbl_planview" class="table table-bordered table-striped">
<?php if ($plan->id_plan->Visible) { // id_plan ?>
	<tr id="r_id_plan">
		<td><span id="elh_plan_id_plan"><?php echo $plan->id_plan->FldCaption() ?></span></td>
		<td<?php echo $plan->id_plan->CellAttributes() ?>>
<span id="el_plan_id_plan" class="control-group">
<span<?php echo $plan->id_plan->ViewAttributes() ?>>
<?php echo $plan->id_plan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $plan->tipo_plan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $plan->time_plan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($plan->st_plan->Visible) { // st_plan ?>
	<tr id="r_st_plan">
		<td><span id="elh_plan_st_plan"><?php echo $plan->st_plan->FldCaption() ?></span></td>
...
<?php echo $plan->st_plan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
