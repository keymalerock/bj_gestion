...
$afiliado_add = NULL; // Initialize page object first

class cafiliado_add extends cafiliado {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_afiliado"] != "") {
				...
				$this->setKey("id_afiliado", $this->id_afiliado->CurrentValue); // Set up key
			} else {
				$this->setKey("id_afiliado", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "afiliadoview.php")
	...
	function LoadDefaultValues() {
		$this->dociden_afiliado->CurrentValue = NULL;
		$this->dociden_afiliado->OldValue = $this->dociden_afiliado->CurrentValue;
		$this->apell_afiliado->CurrentValue = NULL;
		$this->apell_afiliado->OldValue = $this->apell_afiliado->CurrentValue;
		$this->nomb_afiliado->CurrentValue = NULL;
		$this->nomb_afiliado->OldValue = $this->nomb_afiliado->CurrentValue;
		$this->direcc_afiliado->CurrentValue = NULL;
		$this->direcc_afiliado->OldValue = $this->direcc_afiliado->CurrentValue;
		$this->email_afiliado->CurrentValue = NULL;
		$this->email_afiliado->OldValue = $this->email_afiliado->CurrentValue;
		$this->cel_afiliado->CurrentValue = NULL;
		$this->cel_afiliado->OldValue = $this->cel_afiliado->CurrentValue;
		$this->genero_afiliado->CurrentValue = NULL;
		$this->genero_afiliado->OldValue = $this->genero_afiliado->CurrentValue;
		$this->fe_afiliado->CurrentValue = NULL;
		$this->fe_afiliado->OldValue = $this->fe_afiliado->CurrentValue;
		$this->telemerg_afiliado->CurrentValue = NULL;
		$this->telemerg_afiliado->OldValue = $this->telemerg_afiliado->CurrentValue;
		$this->talla_afiliado->CurrentValue = NULL;
		$this->talla_afiliado->OldValue = $this->talla_afiliado->CurrentValue;
		$this->peso_afiliado->CurrentValue = NULL;
		$this->peso_afiliado->OldValue = $this->peso_afiliado->CurrentValue;
		$this->altu_afiliado->CurrentValue = NULL;
		$this->altu_afiliado->OldValue = $this->altu_afiliado->CurrentValue;
		$this->localresdi_afiliado->CurrentValue = NULL;
		$this->localresdi_afiliado->OldValue = $this->localresdi_afiliado->CurrentValue;
		$this->telf_fijo_afiliado->CurrentValue = NULL;
		$this->telf_fijo_afiliado->OldValue = $this->telf_fijo_afiliado->CurrentValue;
		$this->coleg_afiliado->CurrentValue = NULL;
		$this->coleg_afiliado->OldValue = $this->coleg_afiliado->CurrentValue;
		$this->seguro_afiliado->CurrentValue = NULL;
		$this->seguro_afiliado->OldValue = $this->seguro_afiliado->CurrentValue;
		$this->tiposangre_afiliado->CurrentValue = NULL;
		$this->tiposangre_afiliado->OldValue = $this->tiposangre_afiliado->CurrentValue;
		$this->contacto_afiliado->CurrentValue = NULL;
		$this->contacto_afiliado->OldValue = $this->contacto_afiliado->CurrentValue;
		$this->st_afiliado->CurrentValue = 1;
		$this->foto_afiliado->Upload->DbValue = NULL;
		$this->foto_afiliado->OldValue = $this->foto_afiliado->Upload->DbValue;
		$this->foto_afiliado->CurrentValue = NULL; // Clear file related field
		$this->st_notificado->CurrentValue = 1;
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->foto_afiliado);
	...
	function ValidateForm() {
		...
		if (in_array("historial", $DetailTblVar) && $GLOBALS["historial"]->DetailAdd) {
		...
		if (in_array("matricula", $DetailTblVar) && $GLOBALS["matricula"]->DetailAdd) {
		...
		if (in_array("representantes", $DetailTblVar) && $GLOBALS["representantes"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->dociden_afiliado->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(dociden_afiliado = '" . ew_AdjustSql($this->dociden_afiliado->CurrentValue) . "')";
		...
		if ($this->email_afiliado->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(email_afiliado = '" . ew_AdjustSql($this->email_afiliado->CurrentValue) . "')";
			$rsChk = $this->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		if ($this->cel_afiliado->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(cel_afiliado = '" . ew_AdjustSql($this->cel_afiliado->CurrentValue) . "')";
			$rsChk = $this->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// dociden_afiliado
		$this->dociden_afiliado->SetDbValueDef($rsnew, $this->dociden_afiliado->CurrentValue, "", FALSE);

		// apell_afiliado
		$this->apell_afiliado->SetDbValueDef($rsnew, $this->apell_afiliado->CurrentValue, "", FALSE);

		// nomb_afiliado
		$this->nomb_afiliado->SetDbValueDef($rsnew, $this->nomb_afiliado->CurrentValue, "", FALSE);

		// direcc_afiliado
		$this->direcc_afiliado->SetDbValueDef($rsnew, $this->direcc_afiliado->CurrentValue, NULL, FALSE);

		// email_afiliado
		$this->email_afiliado->SetDbValueDef($rsnew, $this->email_afiliado->CurrentValue, NULL, FALSE);

		// cel_afiliado
		$this->cel_afiliado->SetDbValueDef($rsnew, $this->cel_afiliado->CurrentValue, NULL, FALSE);

		// genero_afiliado
		$this->genero_afiliado->SetDbValueDef($rsnew, $this->genero_afiliado->CurrentValue, "", FALSE);

		// fe_afiliado
		$this->fe_afiliado->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fe_afiliado->CurrentValue, 5), ew_CurrentDate(), FALSE);

		// telemerg_afiliado
		$this->telemerg_afiliado->SetDbValueDef($rsnew, $this->telemerg_afiliado->CurrentValue, NULL, FALSE);

		// talla_afiliado
		$this->talla_afiliado->SetDbValueDef($rsnew, $this->talla_afiliado->CurrentValue, NULL, FALSE);

		// peso_afiliado
		$this->peso_afiliado->SetDbValueDef($rsnew, $this->peso_afiliado->CurrentValue, NULL, FALSE);

		// altu_afiliado
		$this->altu_afiliado->SetDbValueDef($rsnew, $this->altu_afiliado->CurrentValue, NULL, FALSE);

		// localresdi_afiliado
		$this->localresdi_afiliado->SetDbValueDef($rsnew, $this->localresdi_afiliado->CurrentValue, NULL, FALSE);

		// telf_fijo_afiliado
		$this->telf_fijo_afiliado->SetDbValueDef($rsnew, $this->telf_fijo_afiliado->CurrentValue, NULL, FALSE);

		// coleg_afiliado
		$this->coleg_afiliado->SetDbValueDef($rsnew, $this->coleg_afiliado->CurrentValue, NULL, FALSE);

		// seguro_afiliado
		$this->seguro_afiliado->SetDbValueDef($rsnew, $this->seguro_afiliado->CurrentValue, NULL, FALSE);

		// tiposangre_afiliado
		$this->tiposangre_afiliado->SetDbValueDef($rsnew, $this->tiposangre_afiliado->CurrentValue, NULL, FALSE);

		// contacto_afiliado
		$this->contacto_afiliado->SetDbValueDef($rsnew, $this->contacto_afiliado->CurrentValue, NULL, FALSE);

		// st_afiliado
		$this->st_afiliado->SetDbValueDef($rsnew, $this->st_afiliado->CurrentValue, NULL, FALSE);
		...
		// st_notificado
		$this->st_notificado->SetDbValueDef($rsnew, $this->st_notificado->CurrentValue, NULL, FALSE);
		if (!$this->foto_afiliado->Upload->KeepFile) {
		...
		if ($AddRow) {
			$this->id_afiliado->setDbValue($conn->Insert_ID());
			$rsnew['id_afiliado'] = $this->id_afiliado->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("historial", $DetailTblVar) && $GLOBALS["historial"]->DetailAdd) {
				$GLOBALS["historial"]->id_afiliado->setSessionValue($this->id_afiliado->CurrentValue); // Set master key
				if (!isset($GLOBALS["historial_grid"])) $GLOBALS["historial_grid"] = new chistorial_grid(); // Get detail page object
				$AddRow = $GLOBALS["historial_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["historial"]->id_afiliado->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("matricula", $DetailTblVar) && $GLOBALS["matricula"]->DetailAdd) {
				$GLOBALS["matricula"]->id_afiliado->setSessionValue($this->id_afiliado->CurrentValue); // Set master key
				if (!isset($GLOBALS["matricula_grid"])) $GLOBALS["matricula_grid"] = new cmatricula_grid(); // Get detail page object
				$AddRow = $GLOBALS["matricula_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["matricula"]->id_afiliado->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("representantes", $DetailTblVar) && $GLOBALS["representantes"]->DetailAdd) {
				$GLOBALS["representantes"]->id_afiliado->setSessionValue($this->id_afiliado->CurrentValue); // Set master key
				if (!isset($GLOBALS["representantes_grid"])) $GLOBALS["representantes_grid"] = new crepresentantes_grid(); // Get detail page object
				$AddRow = $GLOBALS["representantes_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["representantes"]->id_afiliado->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("historial", $DetailTblVar)) {
				...
				if ($GLOBALS["historial_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["historial_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["historial_grid"]->CurrentMode = "add";
					$GLOBALS["historial_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("matricula", $DetailTblVar)) {
				...
				if ($GLOBALS["matricula_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["matricula_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["matricula_grid"]->CurrentMode = "add";
					$GLOBALS["matricula_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("representantes", $DetailTblVar)) {
				...
				if ($GLOBALS["representantes_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["representantes_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["representantes_grid"]->CurrentMode = "add";
					$GLOBALS["representantes_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($afiliado_add)) $afiliado_add = new cafiliado_add();

// Page init
$afiliado_add->Page_Init();

// Page main
$afiliado_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$afiliado_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var afiliado_add = new ew_Page("afiliado_add");
afiliado_add.PageID = "add"; // Page ID
var EW_PAGE_ID = afiliado_add.PageID; // For backward compatibility

// Form object
var fafiliadoadd = new ew_Form("fafiliadoadd");

// Validate form
fafiliadoadd.Validate = function() {
...
fafiliadoadd.Form_CustomValidate = 
...
fafiliadoadd.ValidateRequired = true;
<?php } else { ?>
fafiliadoadd.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
fafiliadoadd.MultiPage = new ew_MultiPage("fafiliadoadd",
...
<?php $afiliado_add->ShowPageHeader(); ?>
<?php
$afiliado_add->ShowMessage();
?>
<form name="fafiliadoadd" id="fafiliadoadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="afiliado">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewStdTable"><tbody><tr><td>
<div class="tabbable" id="afiliado_add">
...
<table id="tbl_afiliadoadd1" class="table table-bordered table-striped">
...
ew_CreateCalendar("fafiliadoadd", "x_fe_afiliado", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<table class="ewGrid" style="width: 100%"><tr><td>
<table id="tbl_afiliadoadd2" class="table table-bordered table-striped">
...
<?php
	if (in_array("historial", explode(",", $afiliado->getCurrentDetailTable())) && $historial->DetailAdd) {
?>
<?php include_once "historialgrid.php" ?>
<?php } ?>
<?php
	if (in_array("matricula", explode(",", $afiliado->getCurrentDetailTable())) && $matricula->DetailAdd) {
?>
<?php include_once "matriculagrid.php" ?>
<?php } ?>
<?php
	if (in_array("representantes", explode(",", $afiliado->getCurrentDetailTable())) && $representantes->DetailAdd) {
?>
