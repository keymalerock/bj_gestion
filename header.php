...
// mostrar datos de sesion   
	if (!empty(CurrentUserID())) {
		 echo '<div style="background-color:#ccc"><strong><font color="red">NOMBRE '.CurrentUserName().' ID '.CurrentUserID().'</font></strong></div>';

	// configurar rotulo de sesion
	switch (CurrentUserLevel())    {
		case '1':
			echo '<div style="background-color:#ccc">AFILIADO</div>';
		break;
		case '2':    
			echo '<div style="background-color:#ccc">EMPLEADO</div>';
		break;
		case '3':
			echo '<div style="background-color:#ccc">JEFE</div>';
		break;   
	}  
}
?>
...
<?php if (!ew_IsMobile()) { ?>
  ...
  <div id="ewHeaderRow" class="ewHeaderRow"><img src="phpimages/logo_bj.jpg" alt="" style="border: 0;"></div>
	<!-- header (end) -->
<?php } ?>
<?php if (ew_IsMobile()) { ?>
