...
function CurrentProjectID() {
	...
	return "{CD332243-68AB-4368-BF10-F24F7E84F4D6}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			$sSql = $v_usuarios->GetSQL($sFilter, "");
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('pass_empleado'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('login_empleado')); // Load user name
						$this->setSessionUserID($rs->fields('id_usuario')); // Load User ID
						if (is_null($rs->fields('id_perfil'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('id_perfil'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sFilter = $v_usuarios->UserIDFilter($this->CurrentUserID);
			$sSql = $v_usuarios->GetSQL($sFilter, '');
