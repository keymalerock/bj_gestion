...
$afiliado_foto_afiliado_blobview = NULL; // Initialize page object first

class cafiliado_foto_afiliado_blobview extends cafiliado {
	...
	// Page object name
	var $PageObjName = 'afiliado_foto_afiliado_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'afiliado');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('afiliado', 'x_foto_afiliado');
		...
		// SQL constructor in afiliado class, afiliadoinfo.php
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('foto_afiliado');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('foto_afiliado')));
				if (trim(strval($this->Recordset->fields('foto_afiliado'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('foto_afiliado') . "\"");
				}
...
// Create page object
if (!isset($afiliado_foto_afiliado_blobview)) $afiliado_foto_afiliado_blobview = new cafiliado_foto_afiliado_blobview();

// Page init
$afiliado_foto_afiliado_blobview->Page_Init();

// Page main
$afiliado_foto_afiliado_blobview->Page_Main();
?>
<?php
$afiliado_foto_afiliado_blobview->Page_Terminate();
?>
