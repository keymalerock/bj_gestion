...
$plan_add = NULL; // Initialize page object first

class cplan_add extends cplan {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_plan"] != "") {
				...
				$this->setKey("id_plan", $this->id_plan->CurrentValue); // Set up key
			} else {
				$this->setKey("id_plan", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "planview.php")
	...
	function LoadDefaultValues() {
		$this->tipo_plan->CurrentValue = NULL;
		$this->tipo_plan->OldValue = $this->tipo_plan->CurrentValue;
		$this->time_plan->CurrentValue = NULL;
		$this->time_plan->OldValue = $this->time_plan->CurrentValue;
		$this->st_plan->CurrentValue = NULL;
		$this->st_plan->OldValue = $this->st_plan->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// tipo_plan
		$this->tipo_plan->SetDbValueDef($rsnew, $this->tipo_plan->CurrentValue, NULL, FALSE);

		// time_plan
		$this->time_plan->SetDbValueDef($rsnew, $this->time_plan->CurrentValue, NULL, FALSE);

		// st_plan
		$this->st_plan->SetDbValueDef($rsnew, $this->st_plan->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($plan_add)) $plan_add = new cplan_add();
...
$plan_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var plan_add = new ew_Page("plan_add");
plan_add.PageID = "add"; // Page ID
var EW_PAGE_ID = plan_add.PageID; // For backward compatibility

// Form object
var fplanadd = new ew_Form("fplanadd");

// Validate form
fplanadd.Validate = function() {
...
fplanadd.ValidateRequired = true;
<?php } else { ?>
fplanadd.ValidateRequired = false; 
<?php } ?>
...
<?php $plan_add->ShowPageHeader(); ?>
<?php
$plan_add->ShowMessage();
?>
<form name="fplanadd" id="fplanadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="plan">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_planadd" class="table table-bordered table-striped">
