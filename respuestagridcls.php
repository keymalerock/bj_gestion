...
$respuesta_grid = NULL; // Initialize page object first

class crespuesta_grid extends crespuesta {
	...
	// Page object name
	var $PageObjName = 'respuesta_grid';

	// Grid form hidden field names
	var $FormName = 'frespuestagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["respuesta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_respuesta->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_id_novedad") && $objForm->HasValue("o_id_novedad") && $this->id_novedad->CurrentValue <> $this->id_novedad->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_empleado") && $objForm->HasValue("o_id_empleado") && $this->id_empleado->CurrentValue <> $this->id_empleado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fe_resp") && $objForm->HasValue("o_fe_resp") && $this->fe_resp->CurrentValue <> $this->fe_resp->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_estado_resp") && $objForm->HasValue("o_estado_resp") && $this->estado_resp->CurrentValue <> $this->estado_resp->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_respuesta->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->id_respuesta->CurrentValue = NULL;
		$this->id_respuesta->OldValue = $this->id_respuesta->CurrentValue;
		...
		$this->fe_resp->CurrentValue = NULL;
		$this->fe_resp->OldValue = $this->fe_resp->CurrentValue;
		$this->estado_resp->CurrentValue = 0;
		$this->estado_resp->OldValue = $this->estado_resp->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->id_respuesta->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->id_novedad->setOldValue($objForm->GetValue("o_id_novedad"));
		...
		if (!$this->fe_resp->FldIsDetailKey) {
			$this->fe_resp->setFormValue($objForm->GetValue("x_fe_resp"));
			$this->fe_resp->CurrentValue = ew_UnFormatDateTime($this->fe_resp->CurrentValue, 5);
		}
		$this->fe_resp->setOldValue($objForm->GetValue("o_fe_resp"));
		if (!$this->estado_resp->FldIsDetailKey) {
			$this->estado_resp->setFormValue($objForm->GetValue("x_estado_resp"));
		}
		$this->estado_resp->setOldValue($objForm->GetValue("o_estado_resp"));
	}
	...
	function RestoreFormValues() {
		...
		$this->fe_resp->CurrentValue = $this->fe_resp->FormValue;
		$this->fe_resp->CurrentValue = ew_UnFormatDateTime($this->fe_resp->CurrentValue, 5);
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_respuesta->CurrentValue = strval($arKeys[0]); // id_respuesta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// fe_resp
			$this->fe_resp->EditCustomAttributes = "";
			$this->fe_resp->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->fe_resp->CurrentValue, 5));
			$this->fe_resp->PlaceHolder = ew_RemoveHtml($this->fe_resp->FldCaption());
			...
			// id_respuesta
			$this->id_respuesta->EditCustomAttributes = "";
			$this->id_respuesta->EditValue = $this->id_respuesta->CurrentValue;
			$this->id_respuesta->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!ew_CheckDate($this->fe_resp->FormValue)) {
			ew_AddMessage($gsFormError, $this->fe_resp->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_novedad
			$this->id_novedad->SetDbValueDef($rsnew, $this->id_novedad->CurrentValue, 0, $this->id_novedad->ReadOnly);

			// id_empleado
			$this->id_empleado->SetDbValueDef($rsnew, $this->id_empleado->CurrentValue, NULL, $this->id_empleado->ReadOnly);

			// fe_resp
			$this->fe_resp->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fe_resp->CurrentValue, 5), NULL, $this->fe_resp->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// fe_resp
		$this->fe_resp->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fe_resp->CurrentValue, 5), NULL, FALSE);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "novedad") {
			...
			if ($GLOBALS["novedad"]->EventCancelled) $this->EventCancelled = TRUE;
		}
